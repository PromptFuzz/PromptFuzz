...
extern "C" int LLVMFuzzerTestOneInput_23(const uint8_t *data, size_t size) {
    ...
    ares_parse_ptr_reply(abuf, alen, addr, addrlen, ptr_family, &ptr_host);
    
    // Event: Query DNS
    const char *query_name = "example.com";
    int dns_class = C_IN;
    int type = T_A;
    ares_query(channel, query_name, dns_class, type, [](void *arg, int status, int timeouts, unsigned char *abuf, int alen) {
        // Callback function implementation
    }, nullptr);
    ...
    ares_getnameinfo(channel, &sa, salen, flags, nameinfo_cb, nullptr);
