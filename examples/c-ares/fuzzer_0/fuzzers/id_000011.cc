...
extern "C" int LLVMFuzzerTestOneInput_11(const uint8_t *data, size_t size) {
    ...
    if (fgets(buffer, sizeof(buffer), in_file)) {
        ...
        if (ares_inet_ntop(AF_INET, buffer, buffer, sizeof(buffer))) {
            ip_address = buffer;
        } else {
            cleanup();
            assert_file_name_closed("output_file");
    ...
    ares_gethostbyaddr(channel, ip_address, strlen(ip_address), AF_INET, [](void* arg, int status, int timeouts, struct hostent* hent) {
