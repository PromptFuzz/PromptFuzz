...
extern "C" int LLVMFuzzerTestOneInput_22(const uint8_t* f_data, size_t f_size) {
    ...
    int dns_class = C_IN;
    int dns_type = T_A;
    ares_query(channel, fuzz_str_3, dns_class, dns_type, [](void *arg, int status, int timeouts, unsigned char *abuf, int alen) {
        // TODO: Handle the callback here
    }, nullptr);
    ...
    ares_getnameinfo(channel, &sa, sizeof(sa), fuzz_int32_t_4, callback, nullptr);
