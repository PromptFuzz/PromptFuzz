...
extern "C" int LLVMFuzzerTestOneInput_26(const uint8_t* f_data, size_t f_size) {
  ...
  ares_set_servers_csv(channel, fuzz_str_1);
  ...
  int error_code = 0;
  const char *error_message = ares_strerror(fuzz_int32_t_2);
  ...
  void *arg = nullptr;
  ares_gethostbyname(channel, fuzz_str_3, fuzz_int32_t_4, host_callback, arg);
