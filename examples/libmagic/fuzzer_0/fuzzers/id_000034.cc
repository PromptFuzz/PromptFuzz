...
extern "C" int LLVMFuzzerTestOneInput_34(const uint8_t *data, size_t size) {
    ...
    vpx_codec_enc_config_default(encoder_if, &encoder_cfg, 0);
    
    // Initialize encoder
    vpx_codec_enc_init_ver(&encoder_ctx, encoder_if, &encoder_cfg, 0, VPX_ENCODER_ABI_VERSION);
    
    // Set stream info with input data
    vpx_codec_peek_stream_info(decoder_if, data, static_cast<unsigned int>(size), &stream_info);
    
    // Initialize decoder
    vpx_codec_dec_init_ver(&decoder_ctx, decoder_if, &decoder_cfg, 0, VPX_DECODER_ABI_VERSION);
    ...
    cx_data_buf.sz = size;
    vpx_codec_set_cx_data_buf(&encoder_ctx, &cx_data_buf, 0, 0);
    
    // Encode frame
    vpx_codec_encode(&encoder_ctx, nullptr, 0, 0, 0, 0);
    
    // Decode frame
    vpx_codec_decode(&decoder_ctx, data, static_cast<unsigned int>(size), nullptr, -1);
