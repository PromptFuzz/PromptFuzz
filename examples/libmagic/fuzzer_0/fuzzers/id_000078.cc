...
extern "C" int LLVMFuzzerTestOneInput_78(const uint8_t *data, size_t size) {
    ...
    vpx_codec_enc_init_multi_ver(&encoder_ctx, vpx_codec_vp9_cx(), &encoder_cfg, 1, encoder_flags, &encoder_dsf, 0);
    ...
    vpx_img_wrap(&image, image_fmt, image_width, image_height, image_stride_align, image_data);
    ...
    vpx_img_set_rect(&image, x, y, w, h);
    ...
    vpx_codec_encode(&encoder_ctx, &image, 0, 0, 0, -1);
    ...
    vpx_codec_dec_init_ver(&decoder_ctx, vpx_codec_vp9_dx(), &decoder_cfg, decoder_flags, 0);
    vpx_codec_decode(&decoder_ctx, data, size, 0, -1);
