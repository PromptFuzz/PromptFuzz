...
extern "C" int LLVMFuzzerTestOneInput_42(const uint8_t *data, size_t size) {
    ...
    // Set the input image
    vpx_image_t img;
    vpx_img_wrap(&img, VPX_IMG_FMT_I420, stream_info.w, stream_info.h, 16, nullptr);

    // Encode the input image
    vpx_codec_encode(&enc_ctx, &img, 0, 0, 0, VPX_DL_GOOD_QUALITY);

    // Decode the input data
    vpx_codec_decode(&dec_ctx, data, size, nullptr, -1);

    // Get the preview frame
    const vpx_image_t *preview_frame = vpx_codec_get_preview_frame(&dec_ctx);

    // Encode the preview frame
    vpx_codec_encode(&enc_ctx, preview_frame, 0, 0, 0, VPX_DL_GOOD_QUALITY);
