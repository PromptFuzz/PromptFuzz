...
extern "C" int LLVMFuzzerTestOneInput_77(const uint8_t *data, size_t size) {
    ...
    vpx_codec_enc_config_default(vpx_codec_vp9_cx(), &enc_config, 0);
    enc_config.g_w = 640;
    enc_config.g_h = 480;

    // Step 2: Initialize libvpx codec
    vpx_codec_enc_init_ver(&codec, vpx_codec_vp9_cx(), &enc_config, 0, VPX_ENCODER_ABI_VERSION);
    ...
    vpx_codec_set_cx_data_buf(&codec, &cx_data_buf, 0, 0);
    ...
    vpx_img_set_rect(&img, 0, 0, 640, 480);
