...
extern "C" int LLVMFuzzerTestOneInput_44(const uint8_t *data, size_t size) {
    ...
    vpx_codec_err_t enc_ret = vpx_codec_enc_config_default(vpx_codec_vp8_cx(), &enc_cfg, 0);
    ...
    enc_ret = vpx_codec_enc_init_ver(&enc_ctx, enc_iface, &enc_cfg, 0, VPX_ENCODER_ABI_VERSION);
    if (enc_ret != VPX_CODEC_OK) {
        // Error handling
        assert_file_closed(&in_file);;
        assert_file_closed(&out_file);;
    ...
    vpx_codec_encode(&enc_ctx, img_ptr, pts, duration, flags, deadline);
    ...
    dec_ret = vpx_codec_decode(&dec_ctx, encoded_data, encoded_data_size, nullptr, -1);
