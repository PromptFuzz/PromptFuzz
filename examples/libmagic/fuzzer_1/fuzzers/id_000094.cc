...
extern "C" int LLVMFuzzerTestOneInput_94(const uint8_t* f_data, size_t f_size) {
    ...
    decoderConfig.w = 640;
    decoderConfig.h = 480;
    vpx_codec_dec_init_ver(&decoder, vpx_codec_vp9_dx(), &decoderConfig, fuzz_int64_t_1, VPX_DECODER_ABI_VERSION);

    // Create an encoder context
    vpx_codec_ctx_t encoder;
    vpx_codec_enc_cfg_t encoderConfig;
    vpx_codec_enc_config_default(vpx_codec_vp9_cx(), &encoderConfig, fuzz_uint32_t_2);
    vpx_codec_enc_init_ver(&encoder, vpx_codec_vp9_cx(), &encoderConfig, fuzz_int64_t_3, VPX_ENCODER_ABI_VERSION);
    ...
    vpx_img_wrap(&image, fmt, width, height, fuzz_uint32_t_4, nullptr);
    ...
    vpx_codec_decode(&decoder, data, size, nullptr, fuzz_int64_t_13);
