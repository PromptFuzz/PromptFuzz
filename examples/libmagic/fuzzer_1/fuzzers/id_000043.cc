...
extern "C" int LLVMFuzzerTestOneInput_43(const uint8_t* f_data, size_t f_size) {
    ...
    vpx_img_wrap(&img, VPX_IMG_FMT_I420, si.w, si.h, fuzz_uint32_t_4, (unsigned char *)data);
    
    // Step 5: Set rectangle on image
    vpx_img_set_rect(&img, fuzz_uint32_t_5, fuzz_uint32_t_6, si.w, si.h);
    
    // Step 6: Allocate memory for decoded image
    vpx_img_alloc(&dec_img, VPX_IMG_FMT_I420, si.w, si.h, 1);
    
    // Step 7: Encode image
    vpx_codec_encode(&enc_ctx, &img, fuzz_int64_t_7, fuzz_uint64_t_8, fuzz_int64_t_9, fuzz_uint64_t_10);
