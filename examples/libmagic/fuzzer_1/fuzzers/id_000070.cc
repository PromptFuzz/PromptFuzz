...
extern "C" int LLVMFuzzerTestOneInput_70(const uint8_t* f_data, size_t f_size) {
    ...
    vpx_codec_err_t res;
    res = vpx_codec_dec_init_ver(&codec_ctx, vpx_codec_vp9_dx(), nullptr, fuzz_int64_t_1, VPX_DECODER_ABI_VERSION);
    if (res != VPX_CODEC_OK) {
    ...
    while ((bytes_read = fread(input_data, 1, sizeof(input_data), in_file)) > 0) {
        ...
        res = vpx_codec_decode(&codec_ctx, input_data, bytes_read, nullptr, fuzz_int64_t_2);
        if (res != VPX_CODEC_OK) {
