...
extern "C" int LLVMFuzzerTestOneInput_58(const uint8_t* f_data, size_t f_size) {
    ...
    dec_cfg.w = width;
    dec_cfg.h = height;
    vpx_codec_dec_init_ver(&codec_ctx, vpx_codec_vp9_dx(), &dec_cfg, fuzz_int64_t_1, VPX_DECODER_ABI_VERSION);

    // Read the input data and decode the frames
    uint8_t *buffer = new uint8_t[size];
    fread(buffer, 1, size, in_file);

    vpx_codec_decode(&codec_ctx, buffer, size, NULL, fuzz_int64_t_2);
    ...
    enc_cfg.g_w = width;
    enc_cfg.g_h = height;
    vpx_codec_enc_init_ver(&enc_ctx, vpx_codec_vp9_dx(), &enc_cfg, fuzz_int64_t_3, VPX_ENCODER_ABI_VERSION);
    
    vpx_codec_encode(&enc_ctx, decoded_frame, fuzz_int64_t_4, fuzz_uint64_t_5, fuzz_int64_t_6, fuzz_uint64_t_7);
