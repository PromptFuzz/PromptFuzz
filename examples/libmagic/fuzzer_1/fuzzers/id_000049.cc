...
extern "C" int LLVMFuzzerTestOneInput_49(const uint8_t* f_data, size_t f_size) {
    ...
    vpx_rational_t dsf;
    vpx_codec_enc_init_multi_ver(&encoder, vpx_codec_vp9_cx(), &enc_cfg, fuzz_int32_t_2, fuzz_int64_t_3, &dsf, VPX_ENCODER_ABI_VERSION);

    // Wrap image
    vpx_image_t image;
    vpx_img_wrap(&image, VPX_IMG_FMT_I420, 1280, 720, fuzz_uint32_t_4, NULL);
    ...
    // Decode
    vpx_codec_decode(&decoder, data, size, NULL, fuzz_int64_t_9);
