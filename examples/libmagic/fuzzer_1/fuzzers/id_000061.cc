...
extern "C" int LLVMFuzzerTestOneInput_61(const uint8_t* f_data, size_t f_size) {
    ...
    vpx_codec_err_t dec_init_result = vpx_codec_dec_init_ver(&ctx, iface, &dec_cfg, fuzz_int64_t_1, fuzz_int32_t_2);
    if (dec_init_result != VPX_CODEC_OK) {
        const char *err_str = vpx_codec_err_to_string(dec_init_result);
        // Handle the error
    }
    
    vpx_codec_err_t enc_init_result = vpx_codec_enc_init_ver(&ctx, encoder_iface, &enc_cfg, fuzz_int64_t_3, fuzz_int32_t_4);
