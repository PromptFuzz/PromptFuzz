...
extern "C" int LLVMFuzzerTestOneInput_24(const uint8_t* f_data, size_t f_size) {
    ...
    if (vpx_codec_enc_init_ver(&ctx, vpx_codec_vp9_cx(), &enc_cfg, fuzz_int64_t_2, VPX_ENCODER_ABI_VERSION) != VPX_CODEC_OK) {
        assert_file_closed(&in_file);;
        assert_file_closed(&out_file);;
        assert_file_closed(&out_file);
	assert_file_closed(&in_file);
    ...
    while (!feof(in_file)) {
        ...
        // Encode the frame
        vpx_codec_encode(&ctx, nullptr, fuzz_int64_t_3, fuzz_uint64_t_4, fuzz_int64_t_5, fuzz_uint64_t_6);
