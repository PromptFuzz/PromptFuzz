...
extern "C" int LLVMFuzzerTestOneInput_86(const uint8_t* f_data, size_t f_size) {
    ...
    vpx_codec_enc_init_ver(&encode_ctx, vpx_codec_vp9_cx(), &enc_cfg, fuzz_int64_t_2, VPX_ENCODER_ABI_VERSION);

    // Encode the image using the encoder context
    vpx_codec_encode(&encode_ctx, &img, fuzz_int64_t_3, fuzz_uint64_t_4, fuzz_int64_t_5, fuzz_uint64_t_6);

    // Create the decoder context
    vpx_codec_ctx_t decode_ctx;
    vpx_codec_dec_init_ver(&decode_ctx, vpx_codec_vp9_dx(), NULL, fuzz_int64_t_7, VPX_DECODER_ABI_VERSION);

    // Decode the encoded data using the decoder context
    vpx_codec_decode(&decode_ctx, data, size, NULL, fuzz_int64_t_8);
