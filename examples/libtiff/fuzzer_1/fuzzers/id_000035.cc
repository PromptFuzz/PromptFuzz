...
extern "C" int LLVMFuzzerTestOneInput_35(const uint8_t* f_data, size_t f_size) {
    ...
    int unlink_result = TIFFUnlinkDirectory(in_tif, fuzz_uint32_t_2);
    ...
    const TIFFField *field = TIFFFindField(in_tif, fuzz_uint32_t_3, data_type);
    ...
    if (field != NULL) {
        ...
        _TIFFmemset(memory, fuzz_int32_t_4, pass_count);
        //...
    ...
    tmsize_t tile_size = TIFFWriteRawTile(in_tif, fuzz_uint32_t_5, tile_data, fuzz_int64_t_6);
    //...
    ...
    read_tile_size = TIFFReadRawTile(in_tif, fuzz_uint32_t_7, read_tile_data, read_tile_size);
