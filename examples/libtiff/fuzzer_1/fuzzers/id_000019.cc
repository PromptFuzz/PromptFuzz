...
extern "C" int LLVMFuzzerTestOneInput_19(const uint8_t* f_data, size_t f_size) {
    ...
    uint32_t* raster;
    int result = TIFFReadRGBAImageOriented(in_tif, width, height, raster, 0, fuzz_int32_t_2);
    if (result == 0) {
        TIFFClose(in_tif);
        assert_file_closed(&in_file);
	return 0;
    ...
    // Read raw tile data
    tmsize_t read_tile_size = TIFFReadRawTile(in_tif, fuzz_uint32_t_3, tile_data, tile_size);
