...
extern "C" int LLVMFuzzerTestOneInput_69(const uint8_t* f_data, size_t f_size) {
    ...
    for (uint32_t stripIndex = 0; stripIndex < stripCount; stripIndex++) {
        tmsize_t bytesRead = TIFFReadEncodedStrip(in_tif, fuzz_uint32_t_3, stripBuffer, stripSize);
        if (bytesRead < 0) {
            _TIFFfree(stripBuffer);
            TIFFClose(out_tif);
            TIFFClose(in_tif);
	...
	return 0;
        ...
        tmsize_t bytesWritten = TIFFWriteRawStrip(out_tif, fuzz_uint32_t_4, stripBuffer, bytesRead);
        if (bytesWritten < 0) {
            _TIFFfree(stripBuffer);
            TIFFClose(out_tif);
            TIFFClose(in_tif);
