...
extern "C" int LLVMFuzzerTestOneInput_45(const uint8_t* f_data, size_t f_size) {
    ...
    tmsize_t read_size = TIFFReadRawTile(in_tif, fuzz_uint32_t_2, tiff_data, tiff_size);
    if (read_size < 0) {
        _TIFFfree(tiff_data);
        TIFFClose(in_tif);
        assert_file_closed(&in_file);
    ...
    tmsize_t write_size = TIFFWriteTile(out_tif, tiff_data, fuzz_uint32_t_4, fuzz_uint32_t_5, fuzz_uint32_t_6, fuzz_uint16_t_7);
    if (write_size < 0) {
        _TIFFfree(tiff_data);
        TIFFClose(in_tif);
        TIFFClose(out_tif);
