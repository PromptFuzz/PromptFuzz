...
extern "C" int LLVMFuzzerTestOneInput_11(const uint8_t* f_data, size_t f_size) {
    ...
    uint32_t nrows = 10;
    uint64_t strip_size = TIFFVStripSize64(in_tif, fuzz_uint32_t_2);
    // Process strip_size further if needed

    const TIFFField* field = TIFFFindField(in_tif, fuzz_uint32_t_3, TIFF_ANY);
    ...
    int set_sub_dir = TIFFSetSubDirectory(in_tif, fuzz_uint64_t_4);
    // Process set_sub_dir further if needed

    double y = 1.23;
    int l_f = 4;
    int log_val = LogL10fromY(y, fuzz_int32_t_5);
