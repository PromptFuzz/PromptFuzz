...
extern "C" int LLVMFuzzerTestOneInput_70(const uint8_t* f_data, size_t f_size) {
    ...
    for (uint32_t strip = 0; strip < num_strips; strip++) {
        tmsize_t bytes_read = TIFFReadEncodedStrip(in_tif, fuzz_uint32_t_3, strip_buf, strip_size);
        tmsize_t bytes_written = TIFFWriteRawStrip(out_tif, fuzz_uint32_t_4, strip_buf, bytes_read);
