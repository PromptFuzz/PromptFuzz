...
extern "C" int LLVMFuzzerTestOneInput_53(const uint8_t* data, size_t size) {
    ...
    for (uint32_t i = 0; i < height; i++) {
        tmsize_t bytes_read = TIFFReadEncodedStrip(in_tif, i, scanline_buffer, scanline_size);
        if (bytes_read < 0) {
	...
	return 0;
        ...
        tmsize_t bytes_written = TIFFWriteRawTile(out_tif, i, scanline_buffer, bytes_read);
        if (bytes_written < 0) {
            _TIFFfree(scanline_buffer);
            TIFFClose(in_tif);
            TIFFClose(out_tif);
