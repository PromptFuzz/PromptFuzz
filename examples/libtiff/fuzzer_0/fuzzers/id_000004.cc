...
extern "C" int LLVMFuzzerTestOneInput_4(const uint8_t* data, size_t size) {
    ...
    const TIFFField* field = TIFFFindField(in_tif, TIFFTAG_PLANARCONFIG, TIFF_ANY);
    if (field) {
    ...
    tmsize_t read_size = TIFFReadRawTile(in_tif, 0, input_tiff_buffer, input_tiff_size);

    // Write the input tiff buffer to the output tiff
    TIFFWriteTile(out_tif, input_tiff_buffer, 0, 0, 0, 0);
