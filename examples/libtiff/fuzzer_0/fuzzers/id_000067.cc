...
extern "C" int LLVMFuzzerTestOneInput_67(const uint8_t* data, size_t size) {
    ...
    uint32_t strip = 0;
    tmsize_t rawStripSize = TIFFRawStripSize(in_tif, strip);
    char errorMessage[1024];
    int rgbaImageOK = TIFFRGBAImageOK(in_tif, errorMessage);
    toff_t gpsDirectoryOffset = 0;
    int readGPSDirectoryResult = TIFFReadGPSDirectory(in_tif, gpsDirectoryOffset);
    float xyzValues[3];
    LogLuv24toXYZ(0, xyzValues);
    uint32_t nrows = 0;
    uint64_t vTileSize64 = TIFFVTileSize64(in_tif, nrows);
