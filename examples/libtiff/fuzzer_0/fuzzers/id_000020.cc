...
extern "C" int LLVMFuzzerTestOneInput_20(const uint8_t *data, size_t size) {
  ...
  if (TIFFGetField(in_tif, TIFFTAG_IMAGEWIDTH, &width) &&
    ...
    if (image_data) {
      ...
      if (TIFFReadRGBAImageOriented(in_tif, width, height, image_data, 0,
                                    ORIENTATION_TOPLEFT)) {
        // Process image data
        // ...
        ...
        if (out_file) {
          // Create output TIFF
          TIFF *out_tif = TIFFStreamOpen("MemTIFF", &out_file);
          if (out_tif) {
            ...
            if (strip_data) {
              ...
              tmsize_t raw_tile_size =
                  TIFFReadRawTile(in_tif, 0, strip_data, strip_size);
              if (raw_tile_size != -1) {
                // Process raw tile data
                // ...

                // Write processed tile data to output TIFF
                TIFFWriteEncodedStrip(out_tif, 0, strip_data, raw_tile_size);
              }
