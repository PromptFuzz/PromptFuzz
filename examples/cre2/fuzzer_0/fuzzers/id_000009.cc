...
extern "C" int LLVMFuzzerTestOneInput_9(const uint8_t *data, size_t size) {
    ...
    cre2_set_add(set, pattern1, strlen(pattern1), NULL, 0);

    const char *pattern2 = "pattern2";
    cre2_set_add(set, pattern2, strlen(pattern2), NULL, 0);
    ...
    int match[2];
    size_t num_matches = cre2_set_match(set, input_text, input_text_len, match, 2);
