...
extern "C" int LLVMFuzzerTestOneInput_16(const uint8_t *data, size_t size) {
    ...
    cre2_regexp_t *rex = cre2_new("pattern", 7, NULL);
    if (rex) {
        char error[256];
        ...
        event += cre2_match(rex, nullptr, 0, 0, 0, CRE2_ANCHOR_START, nullptr, 0);
        event += cre2_replace("pattern", &text_and_target, &rewrite);
        event += cre2_extract("pattern", &text_and_target, &rewrite, &target);
