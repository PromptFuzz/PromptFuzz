...
extern "C" int LLVMFuzzerTestOneInput_18(const uint8_t* f_data, size_t f_size) {
    ...
    // Create regex object
    cre2_regexp_t *rex = cre2_new(fuzz_str_1, static_cast<int>(fuzz_str_sz_1), nullptr);

    // Perform regex matching
    cre2_string_t match;
    if (cre2_easy_match(fuzz_str_2, static_cast<int>(fuzz_str_sz_2), fuzz_str_3, static_cast<int>(fuzz_str_sz_3), &match, 1)) {
