...
extern "C" int LLVMFuzzerTestOneInput_6(const uint8_t* f_data, size_t f_size) {
  ...
  cre2_possible_match_range(re, &min_, &max_, fuzz_int32_t_3);
  ...
  cre2_set_add(set, fuzz_str_4, static_cast<size_t>(fuzz_str_sz_4), nullptr, 0);
  ...
  cre2_set_match(set, fuzz_str_5, static_cast<size_t>(fuzz_str_sz_5), match, match_len);
