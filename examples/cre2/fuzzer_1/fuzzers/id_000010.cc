...
extern "C" int LLVMFuzzerTestOneInput_10(const uint8_t* f_data, size_t f_size) {
    ...
    char error[256];
    if (cre2_set_add(set, fuzz_str_1, static_cast<size_t>(fuzz_str_sz_1), error, sizeof(error)) != 1) {
        // Handle error
    }
    ...
    if (cre2_full_match(fuzz_str_2, &input_string, &full_match_string, 1) == 1) {
        // Handle full match
    }

    // Perform partial matching
    cre2_string_t partial_match_string;
    if (cre2_partial_match(fuzz_str_4, &input_string, &partial_match_string, fuzz_int32_t_5) == 1) {
        // Handle partial match
    }

    // Consume patterns in the input data
    cre2_string_t consumed_string;
    if (cre2_consume(fuzz_str_6, &input_string, &consumed_string, fuzz_int32_t_7) == 1) {
