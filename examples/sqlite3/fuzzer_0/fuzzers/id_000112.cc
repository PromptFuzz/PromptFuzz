...
extern "C" int LLVMFuzzerTestOneInput_112(const uint8_t *data, size_t size) {
    ...
    for (int i = 0; i < paramCount; i++) {
        if (sqlite3_bind_zeroblob64(stmt, i+1, 64) != SQLITE_OK) { // Example of using sqlite3_bind_zeroblob64
    ...
    sqlite3_extended_result_codes(db, 1); // Example of using sqlite3_extended_result_codes
