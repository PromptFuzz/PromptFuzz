...
extern "C" int LLVMFuzzerTestOneInput_78(const uint8_t *data, size_t size) {
    ...
    rc = sqlite3_exec(db, selectDataSQL, NULL, NULL, NULL);
    if (rc != SQLITE_OK) {
        sqlite3_close(db);
        return 0;
    }
    ...
    sqlite3_blob *blob;
    rc = sqlite3_blob_open(db, "main", "test", "name", 1, SQLITE_OPEN_READWRITE, &blob);
    if (rc != SQLITE_OK) {
        sqlite3_close(db);
        return 0;
    }
    ...
    rc = sqlite3_blob_read(blob, blobData, blobSize, 0);
    if (rc != SQLITE_OK) {
