...
extern "C" int LLVMFuzzerTestOneInput_22(const uint8_t *data, size_t size) {
    ...
    rc = sqlite3_prepare_v3(db, "INSERT INTO mytable VALUES(?);", -1, SQLITE_PREPARE_PERSISTENT, &stmt, NULL);
    if (rc != SQLITE_OK) {
        sqlite3_close(db);
        return 0;
    }
    ...
    rc = sqlite3_prepare_v3(db, "SELECT mycolumn FROM mytable;", -1, SQLITE_PREPARE_PERSISTENT, &stmt, NULL);
    if (rc != SQLITE_OK) {
        sqlite3_close(db);
        return 0;
    }
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        rc = sqlite3_blob_open(db, "main", "mytable", "mycolumn", sqlite3_column_int64(stmt, 0), 0, &blob);
        if (rc == SQLITE_OK) {
