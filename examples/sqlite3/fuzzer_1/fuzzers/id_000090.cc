...
extern "C" int LLVMFuzzerTestOneInput_90(const uint8_t* f_data, size_t f_size) {
    ...
    rc = sqlite3_bind_int64(stmt, fuzz_int32_t_2, id);
    if (rc != SQLITE_OK) {
    ...
    int blob_size = 0;
    rc = sqlite3_bind_blob(stmt, fuzz_int32_t_3, blob_data, blob_size, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        sqlite3_finalize(stmt);
        sqlite3_close(db);
        assert_file_closed(&in_file);;
    ...
    rc = sqlite3_exec(db, fuzz_str_4, nullptr, nullptr, nullptr);
    if (rc != SQLITE_OK) {
        sqlite3_close(db);
        assert_file_closed(&in_file);;
        assert_file_closed(&out_file);;
