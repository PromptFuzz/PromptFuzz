...
extern "C" int LLVMFuzzerTestOneInput_73(const uint8_t* f_data, size_t f_size)
{
    ...
    for (int i = 1; i <= numParams; i++) {
        const char *paramName = sqlite3_bind_parameter_name(stmt, fuzz_int32_t_1);
        if (paramName != NULL) {
            sqlite3_bind_text(stmt, fuzz_int32_t_2, paramName, -1, SQLITE_STATIC);
        }
    }
    ...
    while (rc == SQLITE_ROW) {
        ...
        for (int i = 0; i < numCols; i++) {
            int colType = sqlite3_column_type(stmt, fuzz_int32_t_3);
            if (colType == SQLITE_TEXT) {
                const char *colText = (const char *)sqlite3_column_text(stmt, fuzz_int32_t_4);
                if (colText != NULL) {
