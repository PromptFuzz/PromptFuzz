...
extern "C" int LLVMFuzzerTestOneInput_107(const uint8_t *data, size_t size) {
    ...
    // Set IT8 data format
    cmsIT8SetDataFormat(hIT8, 0, "SampleName");
    
    // Get IT8 property
    const char *property = cmsIT8GetProperty(hIT8, "PropertyKey");
    ...
    // Use the data and size parameters passed to LLVMFuzzerTestOneInput_107
