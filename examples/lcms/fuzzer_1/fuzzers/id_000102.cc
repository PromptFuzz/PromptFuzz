...
extern "C" int LLVMFuzzerTestOneInput_102(const uint8_t* f_data, size_t f_size) {
  ...
  char languageCode[3];
  char countryCode[3];
  cmsBool translations = cmsMLUtranslationsCodes(mlu, fuzz_uint32_t_1, languageCode, countryCode);

  // Allocate a profile sequence description
  cmsSEQ *seq = cmsAllocProfileSequenceDescription(context, fuzz_uint32_t_2);
