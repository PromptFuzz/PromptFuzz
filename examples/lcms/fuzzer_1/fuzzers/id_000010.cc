...
extern "C" int LLVMFuzzerTestOneInput_10(const uint8_t* f_data, size_t f_size) {
    ...
    cmsGetProfileInfo(nullProfile, cmsInfoDescription, fuzz_str_1, fuzz_str_2, buffer, bufferSize);
    
    // Create an abstract profile
    cmsHPROFILE abstractProfile = cmsCreateBCHSWabstractProfile(100, 0.5, 0.5, 0.5, 0.5, fuzz_uint32_t_3, fuzz_uint32_t_4);
