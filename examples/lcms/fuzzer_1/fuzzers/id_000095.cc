...
extern "C" int LLVMFuzzerTestOneInput_95(const uint8_t* f_data, size_t f_size) {
    ...
    cmsHPROFILE profile = cmsOpenProfileFromFile("input.icc", fuzz_str_1);

    // Create abstract profile
    cmsHPROFILE abstractProfile = cmsCreateBCHSWabstractProfile(256, 0.5, 1.0, 0.0, 1.0, fuzz_uint32_t_2, fuzz_uint32_t_3);
