...
extern "C" int LLVMFuzzerTestOneInput_0(const uint8_t* f_data, size_t f_size) {
  ...
  cmsHPROFILE proofingProfile = cmsOpenProfileFromFile("proofing.icc", fuzz_str_1);
  cmsHTRANSFORM transform = cmsCreateProofingTransform(inputProfile, TYPE_BGR_8, outputProfile, TYPE_Lab_16, proofingProfile, INTENT_PERCEPTUAL, fuzz_uint32_t_2, 0);
  ...
  char infoBuffer[256];
  cmsGetProfileInfoASCII(inputProfile, cmsInfoManufacturer, fuzz_str_3, fuzz_str_4, infoBuffer, sizeof(infoBuffer));
