...
extern "C" int LLVMFuzzerTestOneInput_96(const uint8_t *data, size_t size) {
  ...
  vpx_codec_err_t status =
    vpx_codec_dec_init_ver(&decoder, vpx_codec_vp9_dx(), nullptr, 0, VPX_DECODER_ABI_VERSION);
  ...
  // Decode input data
  vpx_codec_decode(&decoder, in_data, in_size, nullptr, 0);
  delete[] in_data;
  ...
  while ((decoded_frame = vpx_codec_get_frame(&decoder, &iter)) != nullptr) {
    ...
    vpx_codec_enc_config_default(vpx_codec_vp9_cx(), &encoder_config, 0);
    vpx_codec_enc_init_ver(&encoder, vpx_codec_vp9_cx(), &encoder_config, 0, VPX_ENCODER_ABI_VERSION);

    // Encode frame
    vpx_codec_encode(&encoder, decoded_frame, 0, 1, 0, VPX_DL_GOOD_QUALITY);
