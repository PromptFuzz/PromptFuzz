...
extern "C" int LLVMFuzzerTestOneInput_12(const uint8_t *data, size_t size) {
    ...
    vpx_codec_enc_config_default(vpx_codec_vp8_cx(), &enc_cfg, 0);

    // Create an encoder context
    vpx_codec_ctx_t enc_ctx;
    vpx_codec_enc_init_ver(&enc_ctx, vpx_codec_vp8_cx(), &enc_cfg, 0, VPX_ENCODER_ABI_VERSION);
    ...
    vpx_codec_dec_init_ver(&dec_ctx, vpx_codec_vp8_dx(), &dec_cfg, 0, VPX_DECODER_ABI_VERSION);
    ...
    vpx_codec_encode(&enc_ctx, &img, 0, 1, 0, VPX_DL_REALTIME);
