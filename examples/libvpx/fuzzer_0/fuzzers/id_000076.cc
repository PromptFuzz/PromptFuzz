...
extern "C" int LLVMFuzzerTestOneInput_76(const uint8_t *data, size_t size) {
    ...
    decoder_err = vpx_codec_dec_init_ver(&decoder_ctx, decoder_iface, &decoder_cfg, 0, VPX_DECODER_ABI_VERSION);
    ...
    while (!feof(in_file)) {
        ...
        // Decode the frame
        decoder_err = vpx_codec_decode(&decoder_ctx, buffer, bytes_read, nullptr, -1);
        if (decoder_err != VPX_CODEC_OK) {
            break;
        }
