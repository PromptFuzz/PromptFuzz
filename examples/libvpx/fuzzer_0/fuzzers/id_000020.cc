...
extern "C" int LLVMFuzzerTestOneInput_20(const uint8_t *data, size_t size) {
    ...
    vpx_codec_err_t err = vpx_codec_dec_init_ver(&codec_ctx, codec_iface, &dec_cfg, 0, VPX_DECODER_ABI_VERSION);
    if (err != VPX_CODEC_OK) {
    ...
    while ((bytes_read = fread(buffer, 1, sizeof(buffer), in_file)) > 0) {
        // Decode the data
        err = vpx_codec_decode(&codec_ctx, buffer, bytes_read, NULL, 0);
        if (err != VPX_CODEC_OK) {
