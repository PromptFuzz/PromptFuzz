...
extern "C" int LLVMFuzzerTestOneInput_79(const uint8_t* f_data, size_t f_size) {
    ...
    vpx_codec_enc_init_multi_ver(&codec_ctx, iface, &enc_cfg, 1, fuzz_int64_t_2, &dsf, VPX_ENCODER_ABI_VERSION);

    // Step 3: Set input and output buffers
    vpx_fixed_buf_t buf;
    buf.buf = (void *)data;
    buf.sz = size;
    vpx_codec_set_cx_data_buf(&codec_ctx, &buf, fuzz_uint32_t_3, fuzz_uint32_t_4);
