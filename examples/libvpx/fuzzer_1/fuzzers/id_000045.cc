...
extern "C" int LLVMFuzzerTestOneInput_45(const uint8_t* f_data, size_t f_size) {
    ...
    enc_config.g_w = 640;
    enc_config.g_h = 480;
    vpx_codec_enc_init_ver(&encoder, enc_iface, &enc_config, fuzz_int64_t_1, VPX_ENCODER_ABI_VERSION);
    
    // Step 2: Initialize decoder
    dec_config.w = 640;
    dec_config.h = 480;
    vpx_codec_dec_init_ver(&decoder, dec_iface, &dec_config, fuzz_int64_t_2, VPX_DECODER_ABI_VERSION);
    
    // Step 3: Encode and decode frames
    vpx_codec_encode(&encoder, &raw_frame, fuzz_int64_t_3, fuzz_uint64_t_4, fuzz_int64_t_5, fuzz_uint64_t_6);
    vpx_codec_decode(&decoder, data, size, NULL, fuzz_int64_t_7);
