...
extern "C" int LLVMFuzzerTestOneInput_88(const uint8_t* f_data, size_t f_size) {
    ...
    res = vpx_codec_enc_config_default(vpx_codec_vp8_cx(), &cfg, fuzz_uint32_t_1);
    if (res != VPX_CODEC_OK) {
        assert_file_closed(&in_file);;
        assert_file_closed(&in_file);
	return 0;
    ...
    res = vpx_codec_enc_init_ver(&ctx, vpx_codec_vp8_cx(), &cfg, fuzz_int64_t_2, VPX_ENCODER_ABI_VERSION);
    if (res != VPX_CODEC_OK) {
        assert_file_closed(&in_file);;
        assert_file_closed(&in_file);
	return 0;
    ...
    while (fread(&si, sizeof(si), 1, in_file) == 1) {
        ...
        // Wrap the image data
        vpx_image_t *img_wrap = vpx_img_wrap(&img, fmt, width, height, fuzz_uint32_t_3, img_data);
        if (!img_wrap) {
            free(img_data);
            vpx_codec_destroy(&ctx);
            assert_file_closed(&in_file);;
	...
	return 0;
        ...
        // Encode the frame
        res = vpx_codec_encode(&ctx, img_wrap, fuzz_int64_t_4, fuzz_uint64_t_5, VPX_EFLAG_FORCE_KF, fuzz_uint64_t_6);
        if (res != VPX_CODEC_OK) {
            free(img_data);
            vpx_codec_destroy(&ctx);
            assert_file_closed(&in_file);;
