...
extern "C" int LLVMFuzzerTestOneInput_85(const uint8_t* f_data, size_t f_size) {
    ...
    vpx_codec_dec_init_ver(&dec_ctx, dec_iface, &dec_cfg, fuzz_int64_t_1, VPX_DECODER_ABI_VERSION);
    ...
    vpx_codec_err_t dec_err = vpx_codec_decode(&dec_ctx, in_data, size, nullptr, fuzz_int64_t_2);

    if (dec_err == VPX_CODEC_OK) {
        const vpx_image_t *dec_img = vpx_codec_get_frame(&dec_ctx, &iter);
        if (dec_img) {
            vpx_img_set_rect(img, fuzz_uint32_t_3, fuzz_uint32_t_4, dec_img->d_w, dec_img->d_h);

            vpx_codec_ctx_t enc_ctx;
            vpx_codec_iface_t *enc_iface = vpx_codec_vp9_cx();
            vpx_codec_enc_cfg_t enc_cfg = {0};
            vpx_codec_enc_init_multi_ver(&enc_ctx, enc_iface, &enc_cfg, fuzz_int32_t_5, fuzz_int64_t_6, nullptr, fuzz_int32_t_7);
