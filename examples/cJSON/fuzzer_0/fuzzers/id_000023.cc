    ...
    if (parsed_json) {
        ...
        if (cJSON_IsArray(parsed_json)) {
            ...
            for (int i = 0; i < array_size; i++) {
                ...
                cJSON *item = cJSON_GetArrayItem(parsed_json, i);
                ...
                if (cJSON_IsString(item)) {
                    ...
                    cJSON_AddItemToObject(str_obj, "string", cJSON_CreateString(str_value));
                    
                    // Add the object to the root object
                    cJSON_AddItemToObjectCS(root, "strings", str_obj);
                }
            }
        }
