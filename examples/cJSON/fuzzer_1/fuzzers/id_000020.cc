...
extern "C" int LLVMFuzzerTestOneInput_20(const uint8_t* f_data, size_t f_size) {
    ...
    cJSON_AddNumberToObject(number_obj, fuzz_str_1, 42);

    // Add the number_obj to the root object using cJSON_AddItemReferenceToObject
    cJSON_AddItemReferenceToObject(root, fuzz_str_2, number_obj);

    // Create a new cJSON array using cJSON_AddArrayToObject
    cJSON *array_obj = cJSON_AddArrayToObject(root, fuzz_str_3);
