...
extern "C" int LLVMFuzzerTestOneInput_33(const uint8_t* f_data, size_t f_size) {
    ...
    cJSON *widgetObj = cJSON_GetObjectItem(root, fuzz_str_1);
    if (widgetObj != NULL && cJSON_IsObject(widgetObj)) {
        cJSON *debugObj = cJSON_GetObjectItem(widgetObj, fuzz_str_2);
        if (debugObj != NULL && cJSON_IsString(debugObj)) {
            if (strcmp(debugObj->valuestring, "on") == 0) {
                // Compare two cJSON objects
                cJSON *windowObj = cJSON_GetObjectItem(widgetObj, fuzz_str_3);
                cJSON *textObj = cJSON_GetObjectItem(widgetObj, fuzz_str_4);
                cJSON_bool isSame = cJSON_Compare(windowObj, textObj, fuzz_int32_t_5);
                if (isSame) {
                    // Add a true value to the object
                    cJSON_AddTrueToObject(widgetObj, fuzz_str_6);
                }
            }
        }
    }
