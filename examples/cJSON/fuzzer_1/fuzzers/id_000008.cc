...
extern "C" int LLVMFuzzerTestOneInput_8(const uint8_t* f_data, size_t f_size) {
    ...
    if (widget != nullptr && cJSON_IsObject(widget)) {
        cJSON* text = cJSON_GetObjectItem(widget, fuzz_str_2);
        if (text != nullptr && cJSON_IsObject(text)) {
            cJSON* onMouseUp = cJSON_GetObjectItem(text, fuzz_str_3);
            if (onMouseUp != nullptr && cJSON_IsString(onMouseUp)) {
                cJSON_ReplaceItemInObject(text, "onMouseUp", cJSON_CreateRaw(fuzz_str_4));
            }
        }
    }

    // Print the manipulated JSON data
    char* output_string = cJSON_PrintBuffered(root, fuzz_int32_t_5, fuzz_int32_t_6);
    if (output_string != nullptr) {
        printf("%s\n", output_string);
        free(output_string);
    }

    cJSON_DeleteItemFromObject(root, fuzz_str_7);
    cJSON_DeleteItemFromObject(root, fuzz_str_8);
