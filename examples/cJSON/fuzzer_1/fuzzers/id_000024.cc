...
extern "C" int LLVMFuzzerTestOneInput_24(const uint8_t* f_data, size_t f_size) {
    ...
    cJSON *bool_item = cJSON_GetObjectItem(root, fuzz_str_1);
    if (bool_item != NULL && cJSON_IsBool(bool_item)) {
        ...
        cJSON_AddBoolToObject(root, fuzz_str_2, use_jsp);
    }

    // Use cJSON_GetStringValue to get the string value of an item in the root object
    cJSON *string_item = cJSON_GetObjectItem(root, fuzz_str_3);
    if (string_item != NULL && cJSON_IsString(string_item)) {
        ...
        if (template_name != NULL) {
            ...
            cJSON_AddNullToObject(root, fuzz_str_4);
        }
    }
    ...
    if (array != NULL) {
        cJSON_AddItemToObjectCS(root, fuzz_str_5, array);
    }
