...
extern "C" int LLVMFuzzerTestOneInput_20(const uint8_t* f_data, size_t f_size) {
    ...
    pcap_set_option_string(options, PON_TSTAMP_PRECISION, fuzz_str_1);

    // Step 2: Open pcap file for reading
    FILE* in_file = fmemopen((void*)data, size, "rb");
    handle = pcap_fopen_offline_with_tstamp_precision(in_file, fuzz_uint32_t_2, errbuf);
    ...
    if (pcap_set_datalink(handle, fuzz_int32_t_3) != 0) {
        pcap_close(handle);
        assert_file_closed(&in_file);;
        pcap_free_option(options);
        assert_file_closed(&in_file);
    ...
    pcap_loop(handle, fuzz_int32_t_4, callback, (u_char*)dumper);
