...
extern "C" int LLVMFuzzerTestOneInput_21(const uint8_t* f_data, size_t f_size) {
    ...
    pcap_t* handle = pcap_create(fuzz_str_1, pcap_geterr(handle));
    if (handle == nullptr) {
        return 0;
    }

    pcap_set_datalink(handle, pcap_datalink_name_to_val(fuzz_str_2));
    struct bpf_program fp;
    pcap_setfilter(handle, &fp);

    pcap_tstamp_type_name_to_val(fuzz_str_3);
