...
extern "C" int LLVMFuzzerTestOneInput_18(const uint8_t* f_data, size_t f_size) {
    ...
    pcap_t* pcap_handle = pcap_fopen_offline_with_tstamp_precision(in_file, fuzz_uint32_t_1, errbuf);
    ...
    pcap_set_snaplen(pcap_handle, fuzz_int32_t_2); // Set snapshot length to maximum
  
    struct bpf_program filter_program;
    if (pcap_compile(pcap_handle, &filter_program, fuzz_str_3, fuzz_int32_t_4, fuzz_uint32_t_5) == -1) {
    ...
    int datalink = pcap_datalink_name_to_val(fuzz_str_7);
    ...
    pcap_set_immediate_mode(pcap_handle, fuzz_int32_t_8); // Enable immediate mode
  
    pcap_dumper_t* dumper = pcap_dump_open(pcap_handle, fuzz_str_9);
    if (dumper == NULL) {
