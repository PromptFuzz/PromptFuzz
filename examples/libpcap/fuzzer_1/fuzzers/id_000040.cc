...
extern "C" int LLVMFuzzerTestOneInput_40(const uint8_t* f_data, size_t f_size) {
    ...
    if (pcap_compile(handle, &fp, fuzz_str_1, fuzz_int32_t_2, fuzz_uint32_t_3) == -1) {
    ...
    pcap_set_option_string(options, PON_IO_READ_PLUGIN, fuzz_str_4);
    pcap_set_option_string(options, PON_IO_WRITE_PLUGIN, fuzz_str_5);
