...
extern "C" int LLVMFuzzerTestOneInput_29(const uint8_t* data, size_t size) {
    ...
    if (pcap_set_tstamp_type(handler, PCAP_TSTAMP_ADAPTER) == -1) {
        // handle error
    }
    ...
    if (pcap_sendpacket(handler, packet, packet_size) == -1) {
        // handle error
    }

    // Step 6: Set immediate mode
    if (pcap_set_immediate_mode(handler, 1) == -1) {
        // handle error
    }

    // Step 6: Set timestamp type again
    if (pcap_set_tstamp_type(handler, PCAP_TSTAMP_HOST) == -1) {
        // handle error
    }
