...
extern "C" int LLVMFuzzerTestOneInput_24(const uint8_t* data, size_t size) {
    ...
    pcap_loop(handle, 0, [](u_char *user, const struct pcap_pkthdr *pkthdr, const u_char *packet) {}, NULL);

    // Set the buffer size
    pcap_set_buffer_size(handle, 8192);
    ...
    if (pcap_lookupnet("eth0", &net, &mask, dev) == -1) {
        pcap_close(handle);
        assert_file_closed(&in_file);;
        assert_file_closed(&out_file);;
        assert_file_closed(&out_file);
