...
extern "C" int LLVMFuzzerTestOneInput_36(const uint8_t *data, size_t size) {
  ...
  if (res == 0) {
    ...
    res = tjDecompressToYUV2(decompressor, jpegBuf, size, dstBuf, width, pitch, height, 0);
    if (res != 0) {
      delete[] dstBuf;
      dstBuf = nullptr;
    }
  ...
  res = tjCompressFromYUVPlanes(compressor, srcPlanes, width, strides, height, subsamp, &jpegBuf, &jpegSize, jpegQual, 0);
