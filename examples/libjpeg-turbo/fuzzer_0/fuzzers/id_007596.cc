...
extern "C" int LLVMFuzzerTestOneInput_116(const uint8_t *data, size_t size) {
    ...
    // Load input image
    srcBuf = tj3LoadImage8(handle, "input_file", &width, 0, &height, &pixelFormat);
  
    // Compress image to JPEG
    tjCompressFromYUV(handle, srcBuf, width, 0, height, TJ_GRAYSCALE, &jpegBuf, &jpegSize, 100, 0);
    ...
    tjDecompressToYUVPlanes(handle, jpegData, file_size, dstPlanes, width, strides, height, 0);
