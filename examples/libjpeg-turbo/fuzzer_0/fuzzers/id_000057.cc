...
extern "C" int LLVMFuzzerTestOneInput_2(const uint8_t *data, size_t size) {
  ...
  int result = tjDecompress2(decompressor, jpegBuf, jpegSize, dstBuf, width, 0, height, TJPF_RGB, TJFLAG_FASTDCT);
  if (result != 0) {
    tjFree(jpegBuf);
    tjFree(dstBuf);
    tjDestroy(decompressor);
  ...
  result = tjSaveImage(OUTPUT_FILE, dstBuf, width, 0, height, TJPF_RGB, 0);
  if (result != 0) {
    assert_file_closed(&output_file);;
    tjFree(jpegBuf);
    tjFree(dstBuf);
