...
extern "C" int LLVMFuzzerTestOneInput_115(const uint8_t *data, size_t size) {
    ...
    ret = tjDecompressToYUV2(handle, data, size, dstBuf, width, 1, height, 0);
    if (ret != 0) {
        char *error = tj3GetErrorStr(handle);
        // Handle the error
        free(jpegBuf);
    ...
    ret = tj3Compress16(handle, (unsigned short*)dstBuf, width, 0, height, TJPF_RGB, &jpegBuf, &jpegBufSize);
    if (ret != 0) {
        char *error = tj3GetErrorStr(handle);
        // Handle the error
        free(jpegBuf);
