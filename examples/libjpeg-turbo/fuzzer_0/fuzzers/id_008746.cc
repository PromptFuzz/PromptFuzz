...
extern "C" int LLVMFuzzerTestOneInput_124(const uint8_t *data, size_t size) {
    ...
    unsigned short *image = tj3LoadImage16(nullptr, "input_file", &width, 0, &height, &pixelFormat);
    
    // Step 2: Write the loaded image to a file using tj3SaveImage16
    tj3SaveImage16(nullptr, "output_file", image, width, 0, height, pixelFormat);
    ...
    int yuvPlaneSize = tj3YUVPlaneSize(0, imgWidth, imgWidth, imgHeight, imgSubsamp);
