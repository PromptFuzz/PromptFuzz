...
extern "C" int LLVMFuzzerTestOneInput_61(const uint8_t *data, size_t size) {
  ...
  if (tjDecompress2(handle, jpegBuf, jpegSize, dstBuf, width, 0, height, TJPF_RGB, 0) != 0) {
    tjDestroy(handle);
    free(dstBuf);
    return 0;
  }
  ...
  if (tjCompress2(handle2, dstBuf, width, 0, height, TJPF_RGB, &jpegBuf2, &jpegSize2, TJSAMP_444, 80, 0) != 0) {
    tjDestroy(handle2);
    free(dstBuf);
    tjFree(jpegBuf2);
    return 0;
