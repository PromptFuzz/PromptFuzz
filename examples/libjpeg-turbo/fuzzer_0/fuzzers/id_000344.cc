...
extern "C" int LLVMFuzzerTestOneInput_10(const uint8_t *data, size_t size) {
    ...
    size_t yuvSize = tj3YUVPlaneSize(0, width, yuvStride, yuvHeight, TJ_420);
    ...
    tjDecodeYUV(tjInstance, jpegBuf, 0, TJ_420, yuvBuffer, width, yuvStride, yuvHeight, TJPF_GRAY, 0);

    // Calculate the size of the RGB buffer
    int rgbStride = tjPlaneWidth(0, width, TJ_420);
    int rgbHeight = tjPlaneHeight(0, height, TJ_420);
    size_t rgbSize = tj3YUVPlaneSize(0, width, rgbStride, rgbHeight, TJPF_RGB);
