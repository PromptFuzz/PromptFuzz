...
extern "C" int LLVMFuzzerTestOneInput_101(const uint8_t *data, size_t size) {
   ...
   tj3CompressFromYUV8(compressor, data, width, 1, height, &jpegBuf, &jpegSize);
   ...
   unsigned char *jpegData = tj3LoadImage8(decompressor, "output_file.jpg", &jpegWidth, 1, &jpegHeight, &jpegPixelFormat);
   ...
   tjDecompressToYUV2(decompressor, jpegData, jpegSize, yuvData, yuvWidth, 1, yuvHeight, 0);
