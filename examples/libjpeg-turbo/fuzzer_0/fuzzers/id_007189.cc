...
void  id_007189_decompressToYUV(const uint8_t *jpegData, size_t jpegSize, uint8_t *yuvData, int width, int height) {
...
void  id_007189_loadAndEncodeToYUV(const char *filename, uint8_t *yuvData, int *width, int *height) {
...
extern "C" int LLVMFuzzerTestOneInput_108(const uint8_t *data, size_t size) {
    ...
    uint8_t yuvData[1024 * 1024];  // Change the buffer size as per your requirement
     id_007189_decompressToYUV(data, size, yuvData, 640, 480);  // Change the width and height as per your requirement
    ...
    int width, height;
     id_007189_loadAndEncodeToYUV(filename, yuvData, &width, &height);
