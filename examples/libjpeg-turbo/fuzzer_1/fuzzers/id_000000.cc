...
extern "C" int LLVMFuzzerTestOneInput_0(const uint8_t* f_data, size_t f_size) {
    ...
    tjDecompressToYUV2(handle, jpegBuf, fileSize, dstBuf, width, fuzz_int32_t_1, height, flags);

    // Compress YUV data to JPEG
    width = 640;
    height = 480;
    dstBuf = (unsigned char *)tjAlloc(tjBufSize(width, height, subsamp));
    tjEncodeYUV2(handle, dstBuf, width, fuzz_int32_t_2, height, pixelFormat, jpegBuf, subsamp, flags);
