...
extern "C" int LLVMFuzzerTestOneInput_119(const uint8_t* f_data, size_t f_size) {
    ...
    tj3LoadImage16(handle, "input_file", &width, fuzz_int32_t_1, &height, &pixelFormat);
    ...
    tj3Set(handle, TJXOPT_GRAY, fuzz_int32_t_2);

    // Step 9: Get the YUV plane height
    tj3YUVPlaneHeight(fuzz_int32_t_3, height, TJ_420);
