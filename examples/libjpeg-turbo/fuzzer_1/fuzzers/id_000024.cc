...
extern "C" int LLVMFuzzerTestOneInput_24(const uint8_t* f_data, size_t f_size) {
    ...
    if (tj3Compress16(compressHandle, (const unsigned short *)srcBuf, width, fuzz_int32_t_1, height, pixelFormat, &compressBuf, &compressSize) != 0) {
        // Error handling
    }

    // Get the size of a YUV plane
    int planeWidth = tj3YUVPlaneWidth(0, width, subsamp);
    int planeHeight = tj3YUVPlaneHeight(fuzz_int32_t_2, height, subsamp);
    int planeStride = planeWidth * tjPixelSize[TJPF_GRAY];
    size_t planeSize = tj3YUVPlaneSize(fuzz_int32_t_3, planeWidth, planeStride, planeHeight, subsamp);
