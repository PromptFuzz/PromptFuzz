...
extern "C" int LLVMFuzzerTestOneInput_62(const uint8_t* f_data, size_t f_size) {
  ...
  short *image_data = tj3LoadImage12(nullptr, "input_file", &width, fuzz_int32_t_1, &height, &pixelFormat);

  // Compress the image using libjpeg-turbo APIs
  unsigned char *jpegBuf = nullptr;
  unsigned long jpegSize = 0;
  tjhandle handle = tjInitCompress();
  tjCompressFromYUVPlanes(handle, (const unsigned char **)&image_data, width, nullptr, height, TJSAMP_420, &jpegBuf, &jpegSize, fuzz_int32_t_2, fuzz_int32_t_3);
  tj3SaveImage8(nullptr, "output_file", jpegBuf, width, fuzz_int32_t_4, height, TJPF_RGB);
