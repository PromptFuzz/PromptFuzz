...
int  id_000049_encodeYUVtoJPEG(uint8_t *srcBuf, int width, int pitch, int height, int pixelFormat, uint8_t *dstBuf, int subsamp, int flags) {
...
int  id_000049_decodeJPEGtoYUV(uint8_t *srcBuf, int align, int subsamp, uint8_t *dstBuf, int width, int pitch, int height, int pixelFormat, int flags) {
...
extern "C" int LLVMFuzzerTestOneInput_49(const uint8_t* f_data, size_t f_size) {
    ...
    tjhandle handle = tjInitCompress();
    tj3Set(handle, TJXOP_ROT180, fuzz_int32_t_1);

    int result = tj3Compress8(handle, input_data, width, fuzz_int32_t_2, height, pixelFormat, &jpegBuf, &jpegSize);
    ...
    result = tjDecodeYUV(handle, jpegBuf, fuzz_int32_t_3, subsamp, decodedBuf, width, 0, height, pixelFormat, flags);
