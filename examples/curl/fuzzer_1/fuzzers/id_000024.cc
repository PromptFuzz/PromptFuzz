...
extern "C" int LLVMFuzzerTestOneInput_24(const uint8_t* f_data, size_t f_size) {
    ...
    if (curl) {
        ...
        char *mapped_format = curl_maprintf(fuzz_str_2, 42);
        ...
        if (mime) {
            ...
            curl_mime_name(part, fuzz_str_3);
            
            // Step 23: Set the data of the mime part
            const char *mime_data = "Multipart Data";
            curl_mime_data(part, fuzz_str_4, static_cast<size_t>(fuzz_str_sz_4));
