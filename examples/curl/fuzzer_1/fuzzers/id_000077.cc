...
extern "C" int LLVMFuzzerTestOneInput_77(const uint8_t* f_data, size_t f_size) {
    ...
    curl_mime_filename(part, fuzz_str_1);

    // Use curl_pushheader_byname to get the header by name
    char *header_value = curl_pushheader_byname(pushheaders, fuzz_str_2);

    // Use curl_easy_nextheader to get the next header
    header = curl_easy_nextheader(curl, fuzz_uint32_t_3, CURLHEADER_SEPARATE, header);
