...
void  id_000105_performCurlRequestAndReceiveFrames(const uint8_t *data, size_t size) {
...
void  id_000105_addSubpartsToMimePart() {
    CURL *curl = curl_easy_init();
    if (curl) {
...
void  id_000105_setUpMultiHandleForMultipleSockets() {
...
void  id_000105_pauseCurlTransfer() {
    CURL *curl = curl_easy_init();
    if (curl) {
...
void  id_000105_receiveWebSocketFramesUsingCurlHandle(const uint8_t *data, size_t size) {
    CURL *curl = curl_easy_init();
    if (curl) {
        // Set up CURL options
...
extern "C" int LLVMFuzzerTestOneInput_105(const uint8_t* f_data, size_t f_size) {
    ...
    // Call the events with the provided data and size
     id_000105_performCurlRequestAndReceiveFrames(data, size);
     id_000105_addSubpartsToMimePart();
     id_000105_setUpMultiHandleForMultipleSockets();
     id_000105_pauseCurlTransfer();
     id_000105_receiveWebSocketFramesUsingCurlHandle(data, size);
