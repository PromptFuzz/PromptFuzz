...
size_t  id_000017_ReadData(void *ptr, size_t size, size_t nmemb, void *stream) {
  FILE *in_file = (FILE *)stream;
  return fread(ptr, size, nmemb, in_file);
}

// Function to write the output data
size_t  id_000017_WriteData(void *ptr, size_t size, size_t nmemb, void *stream) {
...
extern "C" int LLVMFuzzerTestOneInput_17(const uint8_t* f_data, size_t f_size) {
  ...
  curl_easy_setopt(curl, CURLOPT_READFUNCTION,  id_000017_ReadData);
  curl_easy_setopt(curl, CURLOPT_READDATA, in_file);

  // Set the output file as the data sink
  curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION,  id_000017_WriteData);
