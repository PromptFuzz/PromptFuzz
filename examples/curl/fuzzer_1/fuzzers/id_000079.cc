...
extern "C" int LLVMFuzzerTestOneInput_79(const uint8_t* f_data, size_t f_size) {
    ...
    while ((bytesRead = fread(buffer, 1, sizeof(buffer), in_file)) > 0) {
        ...
        CURLcode encoderResult = curl_mime_encoder(nullptr, fuzz_str_1);
        int isEqual = curl_strnequal("string1", fuzz_str_2, 7);
        struct curl_header *nextHeader = curl_easy_nextheader(curl, fuzz_uint32_t_3, fuzz_int32_t_4, nullptr);
