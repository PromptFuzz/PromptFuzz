...
extern "C" int LLVMFuzzerTestOneInput_98(const uint8_t *data, size_t size) {
    ...
    // curl_url_set
    curl_url_set(handle, CURLUPART_URL, "https://www.example.com", 0);
    curl_url_set(handle, CURLUPART_PORT, "80", 0);
    
    // curl_url_get
    CURLUcode res = curl_url_get(handle, CURLUPART_URL, &part, 0);
    
    // curl_global_trace
    curl_global_trace("config.txt");
