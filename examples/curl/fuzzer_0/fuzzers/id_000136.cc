...
CURLcode  id_000136_SendWebSocketData(CURL *curl, const void *buffer, size_t buflen, size_t *sent, curl_off_t fragsize, unsigned int flags) {
    return curl_ws_send(curl, buffer, buflen, sent, fragsize, flags);
}

// Event: Print error message
void  id_000136_PrintErrorMessage(const char *format, CURLMcode code) {
    curl_mprintf(format, code);
}

// Event: Duplicate an easy handle
CURL * id_000136_DuplicateEasyHandle(CURL *curl) {
    return curl_easy_duphandle(curl);
}

// Event: Set data on a MIME part
CURLcode  id_000136_SetMimeData(curl_mimepart *part, const char *data, size_t datasize) {
    return curl_mime_data(part, data, datasize);
}

// Event: Get error message for a multi handle
const char * id_000136_GetMultiErrorMessage(CURLMcode code) {
    return curl_multi_strerror(code);
}

// Event: Perform multi socket operation
CURLMcode  id_000136_MultiSocketOperation(CURLM *multi_handle, curl_socket_t s, int *running_handles) {
...
extern "C" int LLVMFuzzerTestOneInput_5(const uint8_t *data, size_t size) {
    ...
    curl_multi_setopt(multi_handle, CURLMOPT_SOCKETFUNCTION,  id_000136_MultiSocketOperation);
    ...
    while ((bytesRead = fread(buffer, 1, sizeof(buffer), in_file)) > 0) {
         id_000136_SetMimeData(mime_part, buffer, bytesRead);
    }
