...
extern "C" int LLVMFuzzerTestOneInput_10(const uint8_t* f_data, size_t f_size) {
    ...
    gzFile gz_file = gzopen("input_file", fuzz_str_1);
    if (gz_file == nullptr) {
        assert_file_closed(&in_file);;
    ...
    int gz_result = gzbuffer(gz_file, fuzz_uint32_t_2);
    off64_t gz_offset = gztell64(gz_file);
    z_stream strm;
    strm.zalloc = nullptr;
    strm.zfree = nullptr;
    inflateSync(&strm);
    int gz_flush_result = gzflush(gz_file, fuzz_int32_t_3);
    ...
    uLong adler_result = adler32_combine(fuzz_uint64_t_4, fuzz_uint64_t_5, gz_offset);
