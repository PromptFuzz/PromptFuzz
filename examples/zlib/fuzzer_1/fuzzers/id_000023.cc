...
extern "C" int LLVMFuzzerTestOneInput_23(const uint8_t* f_data, size_t f_size) {
    ...
    compress2(deflate_buffer, &deflate_buffer_size, (const Bytef *)data, size, fuzz_int32_t_1);

    // gzseek
    gzFile gzfile = gzopen(input_file, fuzz_str_2);
    if (!gzfile) {
    ...
    off_t offset = gzseek(gzfile, 0L, fuzz_int32_t_3);
    gzclose(gzfile);

    // gzgetc
    gzfile = gzopen(input_file, fuzz_str_4);
    if (!gzfile) {
        delete[] deflate_buffer;
        assert_file_closed(&in_file);;
        assert_file_closed(&out_file);;
    ...
    // gzbuffer
    gzfile = gzopen(input_file, fuzz_str_5);
    if (!gzfile) {
        delete[] deflate_buffer;
        assert_file_closed(&in_file);;
        assert_file_closed(&out_file);;
    ...
    gzbuffer(gzfile, fuzz_uint32_t_6);
    gzclose(gzfile);
    ...
    // crc32_z
    uLong crc_value = crc32_z(fuzz_uint64_t_7, (const Bytef *)data, size);
