...
extern "C" int LLVMFuzzerTestOneInput_80(const uint8_t* f_data, size_t f_size) {
    ...
    if (inflateInit_(&strm, fuzz_str_1, sizeof(z_stream)) != Z_OK) {
    ...
    gzFile gz_in = gzopen("input_file", fuzz_str_2);
    ...
    gzFile gz_out = gzopen("output_file", fuzz_str_3);
    ...
    gzrewind(gz_in);
    inflatePrime(&strm, 16, fuzz_int32_t_4);
    gzdirect(gz_in);
	...
	return 0;
        ...
        if (inflate(&strm, fuzz_int32_t_5) != Z_OK) {
