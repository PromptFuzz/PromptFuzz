...
extern "C" int LLVMFuzzerTestOneInput_69(const uint8_t *data, size_t size) {
    ...
    int handle_result = png_handle_as_unknown(png_ptr, (png_const_bytep)"chunk_name");
    ...
    png_image_write_to_memory(&image, NULL, &memory_bytes, 0, NULL, 0, NULL);
    void *memory = malloc(memory_bytes);
    png_image_write_to_memory(&image, memory, &memory_bytes, 0, NULL, 0, NULL);
