...
extern "C" int LLVMFuzzerTestOneInput_113(const uint8_t* f_data, size_t f_size) {
    ...
    png_set_user_limits(png_ptr, fuzz_uint32_t_1, fuzz_uint32_t_2);

    // Set unknown chunk location
    png_set_unknown_chunk_location(png_ptr, info_ptr, fuzz_int32_t_3, fuzz_int32_t_4);

    // Strip alpha channel
    png_set_strip_alpha(png_ptr);

    // Flush the data - for testing purposes only
    png_set_flush(png_ptr, fuzz_int32_t_5);

    // Save int_32 value to a byte buffer
    png_byte buf[4];
    png_save_int_32(buf, fuzz_int32_t_6);
