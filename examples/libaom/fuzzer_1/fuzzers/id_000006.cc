...
extern "C" int LLVMFuzzerTestOneInput_6(const uint8_t* f_data, size_t f_size) {
  ...
  aom_codec_enc_config_default(codec_iface, &codec_cfg, fuzz_uint32_t_1);
  if (aom_codec_enc_init_ver(&codec_ctx, codec_iface, &codec_cfg, fuzz_int64_t_2, AOM_ENCODER_ABI_VERSION) != AOM_CODEC_OK) {
    assert_file_closed(&in_file);;
    assert_file_closed(&out_file);;
    assert_file_name_closed("output_file");
	assert_file_closed(&out_file);
  ...
  // Create an image
  aom_image_t *image = aom_img_alloc(NULL, AOM_IMG_FMT_I420, fuzz_uint32_t_3, fuzz_uint32_t_4, 16);
  if (!image) {
    assert_file_closed(&in_file);;
    assert_file_closed(&out_file);;
  ...
  aom_img_set_rect(image, fuzz_uint32_t_5, fuzz_uint32_t_6, fuzz_uint32_t_7, fuzz_uint32_t_8, fuzz_uint32_t_9);

  // Encode the image
  aom_codec_pts_t pts = 0;
  aom_codec_encode(&codec_ctx, image, pts, fuzz_uint64_t_10, fuzz_int64_t_11);
