...
extern "C" int LLVMFuzzerTestOneInput_15(const uint8_t* f_data, size_t f_size) {
  ...
  aom_codec_enc_init_ver(&encoder, encoder_iface, &encoder_cfg, fuzz_int64_t_2, AOM_ENCODER_ABI_VERSION);
  ...
  aom_codec_set_cx_data_buf(&encoder, &encoder_buf, 0, fuzz_uint32_t_3);

  // Create input image
  aom_image_t input_image;
  aom_img_alloc(&input_image, AOM_IMG_FMT_I420, fuzz_uint32_t_4, fuzz_uint32_t_5, 16);
