...
extern "C" int LLVMFuzzerTestOneInput_12(const uint8_t* f_data, size_t f_size) {
    ...
    aom_codec_err_t res = aom_codec_dec_init_ver(&codec, iface, &dec_cfg, fuzz_int64_t_1, AOM_DECODER_ABI_VERSION);
    if (res != AOM_CODEC_OK) {
    ...
    res = aom_codec_set_cx_data_buf(&codec, &input_buf, 0, fuzz_uint32_t_2);
    if (res != AOM_CODEC_OK) {
