...
extern "C" int LLVMFuzzerTestOneInput_22(const uint8_t* f_data, size_t f_size) {
    ...
    aom_codec_dec_init_ver(&decoder, decoder_iface, &decoder_cfg, fuzz_int64_t_1, AOM_DECODER_ABI_VERSION);

    // Step 2: Create an encoder context
    aom_codec_ctx_t encoder;
    aom_codec_iface_t *encoder_iface = aom_codec_av1_cx();
    aom_codec_enc_cfg_t encoder_cfg;
    aom_codec_enc_config_default(encoder_iface, &encoder_cfg, fuzz_uint32_t_2);
    aom_codec_enc_init_ver(&encoder, encoder_iface, &encoder_cfg, fuzz_int64_t_3, AOM_ENCODER_ABI_VERSION);
    ...
    aom_codec_encode(&encoder, decoded_frame, fuzz_int64_t_4, fuzz_uint64_t_5, fuzz_int64_t_6);
