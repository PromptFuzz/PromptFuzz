...
extern "C" int LLVMFuzzerTestOneInput_38(const uint8_t* f_data, size_t f_size) {
    ...
    aom_codec_enc_init_ver(&codec_ctx, codec_iface, &codec_cfg, fuzz_int64_t_1, AOM_ENCODER_ABI_VERSION);

    // Set data buffer
    aom_fixed_buf_t buf;
    buf.buf = (void*)data;
    buf.sz = size;
    aom_codec_set_cx_data_buf(&codec_ctx, &buf, 0, fuzz_uint32_t_2);
    ...
    aom_codec_encode(&codec_ctx, &img, pts, fuzz_uint64_t_3, flags);
