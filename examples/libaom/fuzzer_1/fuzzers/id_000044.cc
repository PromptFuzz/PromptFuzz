...
extern "C" int LLVMFuzzerTestOneInput_44(const uint8_t* f_data, size_t f_size) {
  ...
  if (aom_codec_dec_init_ver(&codec_ctx, iface, &cfg, fuzz_int64_t_1, AOM_DECODER_ABI_VERSION)) {
    assert_file_closed(&in_file);;
    assert_file_closed(&in_file);
  ...
  const aom_metadata_t *metadata_ptr = aom_img_get_metadata(&img, fuzz_uint64_t_2);
