...
extern "C" int LLVMFuzzerTestOneInput_9(const uint8_t* f_data, size_t f_size) {
    ...
    aom_codec_err_t decErr = aom_codec_dec_init_ver(&ctx, aom_codec_av1_dx(), &decCfg, fuzz_int64_t_1, AOM_DECODER_ABI_VERSION);
    ...
    aom_codec_err_t encErr = aom_codec_enc_config_default(aom_codec_av1_cx(), &encCfg, fuzz_uint32_t_2);
    if (encErr != AOM_CODEC_OK) {
        aom_codec_destroy(&ctx);
        return 0;
    }
    aom_codec_set_option(&ctx, fuzz_str_3, fuzz_str_4);

    // Step 1: Event - Set CX data buffer
    aom_fixed_buf_t cxDataBuf;
    aom_codec_set_cx_data_buf(&ctx, &cxDataBuf, 0, fuzz_uint32_t_5);
    ...
    aom_metadata_t *metadata = aom_img_metadata_alloc(fuzz_uint32_t_6, data, size, AOM_MIF_ANY_FRAME);
    ...
    aom_img_set_rect(&img, fuzz_uint32_t_7, fuzz_uint32_t_8, fuzz_uint32_t_9, fuzz_uint32_t_10, fuzz_uint32_t_11);
