...
extern "C" int LLVMFuzzerTestOneInput_43(const uint8_t* f_data, size_t f_size) {
    ...
    aom_img_add_metadata(&img, fuzz_uint32_t_1, metadata_payload, metadata_size, insert_flag);
    const aom_metadata_t *metadata = aom_img_get_metadata(&img, fuzz_uint64_t_2);
    const char *error_detail = aom_codec_error_detail(&codec_ctx);
    const char *error = aom_codec_error(&codec_ctx);
    aom_metadata_t *metadata_alloc = aom_img_metadata_alloc(fuzz_uint32_t_3, metadata_payload, metadata_size, insert_flag);
