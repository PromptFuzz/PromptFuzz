...
extern "C" int LLVMFuzzerTestOneInput_27(const uint8_t* f_data, size_t f_size) {
  ...
  aom_codec_enc_init_ver(&encoder, encoder_iface, &encoder_cfg, fuzz_int64_t_1, AOM_DECODER_ABI_VERSION);

  // Step 3: Allocate input image frame
  aom_image_t input_image;
  aom_img_alloc(&input_image, AOM_IMG_FMT_I420, fuzz_uint32_t_2, fuzz_uint32_t_3, 16);

  // Step 4: Set input image rectangle
  aom_img_set_rect(&input_image, fuzz_uint32_t_4, fuzz_uint32_t_5, fuzz_uint32_t_6, fuzz_uint32_t_7, fuzz_uint32_t_8);

  // Step 5: Get metadata from input image
  const aom_metadata_t *metadata = aom_img_get_metadata(&input_image, fuzz_uint64_t_9);
  if (metadata) {
  ...
  aom_codec_encode(&encoder, &input_image, pts, fuzz_uint64_t_10, flags);
