...
extern "C" int LLVMFuzzerTestOneInput_34(const uint8_t* f_data, size_t f_size) {
    ...
    aom_codec_enc_init_ver(NULL, iface, &cfg, fuzz_int64_t_1, AOM_ENCODER_ABI_VERSION);

    aom_image_t img;
    aom_img_alloc(&img, AOM_IMG_FMT_I420, fuzz_uint32_t_2, fuzz_uint32_t_3, 16);
    ...
    aom_codec_set_cx_data_buf(NULL, &cx_buf, 0, fuzz_uint32_t_4);

    aom_codec_ctx_t ctx;
    aom_codec_enc_init_ver(&ctx, iface, &cfg, fuzz_int64_t_5, AOM_ENCODER_ABI_VERSION);
    aom_codec_encode(&ctx, &img, fuzz_int64_t_6, fuzz_uint64_t_7, fuzz_int64_t_8);
