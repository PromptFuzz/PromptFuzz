...
extern "C" int LLVMFuzzerTestOneInput_5(const uint8_t *data, size_t size) {
    ...
    aom_image_t image;
    aom_img_alloc(&image, AOM_IMG_FMT_I420, 320, 240, 32);
    ...
    aom_codec_enc_config_default(iface, &enc_cfg, 0);
    aom_codec_enc_init_ver(&codec_ctx, iface, &enc_cfg, 0, AOM_DECODER_ABI_VERSION);

    // Set custom data buffer
    aom_fixed_buf_t custom_data;
    custom_data.buf = (void *)data;
    custom_data.sz = size;
    aom_codec_set_cx_data_buf(&codec_ctx, &custom_data, 0, 0);
