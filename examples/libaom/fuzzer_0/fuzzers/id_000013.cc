...
extern "C" int LLVMFuzzerTestOneInput_13(const uint8_t *data, size_t size) {
    ...
    codec_err = aom_codec_dec_init_ver(&codec_ctx, aom_codec_av1_dx(), &dec_cfg, 0, AOM_DECODER_ABI_VERSION);
    ...
    // Step 2: Set the required configuration options
    enc_err = aom_codec_enc_init_ver(&encoder_ctx, encoder_iface, &enc_cfg, 0, AOM_ENCODER_ABI_VERSION);
    if (enc_err != AOM_CODEC_OK) {
        assert_file_closed(&out_file);;
        assert_file_closed(&in_file);;
        assert_file_name_closed("output_file");
