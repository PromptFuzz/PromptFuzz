...
extern "C" int LLVMFuzzerTestOneInput_29(const uint8_t *data, size_t size) {
    ...
    aom_codec_err_t ret = aom_codec_enc_init_ver(&encoder, iface, &cfg, 0, AOM_ENCODER_ABI_VERSION);
    ...
    aom_img_set_rect(&img, 0, 0, width, height, 16);

    // Encode the image
    aom_codec_pts_t pts = 0;
    aom_enc_frame_flags_t flags = 0;
    ret = aom_codec_encode(&encoder, &img, pts, 0, flags);
