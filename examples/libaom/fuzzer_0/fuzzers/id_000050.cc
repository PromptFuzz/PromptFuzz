...
extern "C" int LLVMFuzzerTestOneInput_50(const uint8_t* data, size_t size) {
    ...
    aom_codec_enc_init_ver(&encoder, encoder_iface, &encoder_cfg, 0, AOM_CODEC_ABI_VERSION);
    
    aom_codec_set_frame_buffer_functions(&encoder, nullptr, nullptr, nullptr);
    aom_codec_encode(&encoder, &img, 0, 0, 0);

    // Decode the encoded data
    aom_codec_ctx_t decoder;
    aom_codec_iface_t* decoder_iface = aom_codec_av1_dx();
    aom_codec_dec_cfg_t decoder_cfg;
    aom_codec_dec_init_ver(&decoder, decoder_iface, &decoder_cfg, 0, AOM_CODEC_ABI_VERSION);
