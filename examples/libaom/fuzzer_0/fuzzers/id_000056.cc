...
extern "C" int LLVMFuzzerTestOneInput_56(const uint8_t *data, size_t size) {
    ...
    res = aom_codec_enc_init_ver(&encoder, encoder_iface, &encoder_cfg, 0, AOM_ENCODER_ABI_VERSION);
    if (res != AOM_CODEC_OK) {
        assert_file_closed(&in_file);;
        assert_file_closed(&out_file);;
        assert_file_name_closed("output_file");
    ...
    // Set image rectangle
    aom_img_set_rect(image, 0, 0, encoder_cfg.g_w, encoder_cfg.g_h, 0);
    ...
    while (!feof(in_file)) {
        ...
        // Encode the frame
        res = aom_codec_encode(&encoder, image, 0, 1, 0);
        if (res != AOM_CODEC_OK) {
            break;
        }
