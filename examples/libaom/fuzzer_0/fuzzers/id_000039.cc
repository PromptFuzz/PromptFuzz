...
extern "C" int LLVMFuzzerTestOneInput_39(const uint8_t *data, size_t size) {
   ...
   aom_codec_enc_config_default(enc_iface, &enc_cfg, 0);
   aom_codec_enc_init_ver(&enc_ctx, enc_iface, &enc_cfg, 0, AOM_ENCODER_ABI_VERSION);
   aom_codec_set_option(&enc_ctx, "speed", "6");
   aom_codec_set_option(&enc_ctx, "threads", "8");
   aom_codec_set_option(&enc_ctx, "bit-depth", "8");
   aom_fixed_buf_t output_buf = { 0, 0 };
   aom_codec_set_cx_data_buf(&enc_ctx, &output_buf, 0, 4);
   aom_codec_pts_t pts = 0;
   aom_enc_frame_flags_t flags = 0;
   aom_codec_encode(&enc_ctx, NULL, pts, 1, flags);
